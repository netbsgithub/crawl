...
class MySQLStoreCnblogsPipeline(object):
	...
	if ret:
	    conn.execute("""
	    	update cnblogsinfo set title = %s, description = %s, link = %s, listUrl = %s,content = %s, html = %s, updated = %s where linkmd5id = %s	
	    """, (item['title'], item['desc'], item['link'], item['listUrl'],item['content'],item['html'], now, linkmd5id))
	    #print """
	    ...
	    conn.execute("""
		insert into cnblogsinfo(linkmd5id, title, description, link, listUrl,content,html, updated) 
		values(%s, %s, %s, %s, %s, %s, %s, %s)
	    """, (linkmd5id, item['title'], item['desc'], item['link'], item['listUrl'], item['content'],item['html'],now))
	    #print """
